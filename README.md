...
| **Header-Aware API Discovery** | Scans public `.h` files to find the real API surface. Resolves typedefs, scores struct-pointer APIs, detects init/cleanup lifecycle patterns. Finds `compress`/`uncompress` instead of utility functions. |
| **5-Second Sanity Gate** | Every synthesized harness is run for 5 seconds before acceptance. Dead harnesses (0 edges) are rejected instantly — no wasted fuzzing iterations. |
| **Self-Correction ReAct Loop** | When a harness crashes, GDB extracts the backtrace and feeds it back to the LLM. The agent fixes its own initialization errors autonomously. |
| **Type Extraction & Build Resolution** | Automatically extracts struct/typedef definitions from headers and discovers library paths. The LLM sees exact type layouts; compilation errors are auto-fixed. |
