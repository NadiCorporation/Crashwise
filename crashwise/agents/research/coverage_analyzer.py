...
_BLOCKER_PATTERNS: list[tuple[BlockerType, re.Pattern[str], str]] = [
    ...
    (
        ...
        re.compile(
            r"switch\s*\(\s*(?:state|status|step|phase|mode|stage|type)\s*\)",
            re.IGNORECASE,
        ),
        "State machine switch dispatch",
    ),
    (
        BlockerType.STATE_MACHINE,
        re.compile(
            r"if\s*\(\s*(?:state|status|step|phase|mode|stage)\s*(?:==|!=)\s*\w+\s*\)",
            re.IGNORECASE,
        ),
    ...
    (
        BlockerType.STATE_MACHINE,
        re.compile(
            r"case\s+(?:[A-Z][A-Z0-9_]{2,})\s*:",
        ),
        "Enum-style case label in state dispatch",
    ),
    (
...
async def analyze_coverage(
    ...
    *,
    ...
    # Operation Hydra: if parsers returned empty sets (AFL++/libFuzzer aggregate-only),
    # treat as "no line-level data available" and use static analysis.
    has_line_level_coverage = bool(hit) or bool(missed)
    ...
    # 3. Identify blockers in missed regions (only if we have REAL line data).
    blockers: list[CoverageBlocker] = []
    if has_line_level_coverage:
    ...
    # 4. If no line-level coverage data, do static analysis on all conditional lines.
    if not has_line_level_coverage:
...
def _parse_afl_coverage_summary(text: str) -> tuple[set[int], set[int]]:
    """Parse AFL++ coverage summary — aggregate only, NO synthetic lines.

    Operation Hydra Fix 2: AFL++ does not provide line-level coverage.
    We extract aggregate edge counts for reporting but return EMPTY sets
    to signal that line-level blocker identification is not possible.
    Returning fake line numbers was causing garbage blocker detection.
    """
    # We still parse the data for logging/metrics, but do NOT fabricate lines.
    return set(), set()


def _parse_libfuzzer_coverage_summary(text: str) -> tuple[set[int], set[int]]:
    """Parse libFuzzer coverage summary — aggregate only, NO synthetic lines.

    Operation Hydra Fix 2: libFuzzer reports edge/feature counts but not
    which SOURCE LINES were hit. Fabricating line numbers caused the
    blocker identification to look at random source lines.
    Returns empty sets — callers must use static analysis fallback.
    """
    return set(), set()
...
# ── Dictionary generation for fuzzer token injection ─────────────────────────

# Regex to extract string/hex/integer literals from source code comparisons.
_LITERAL_RE = re.compile(
    r'(?:memcmp|strcmp|strncmp|memcpy)\s*\([^,]*,\s*"([^"]+)"'
    r"|"
    r'(?:==|!=)\s*(0x[0-9a-fA-F]+|\d{2,}|"[^"]+")'
    r"|"
    r"(?:==|!=)\s*'([^']+)'"
)


def generate_dictionary(source_path: Path, blockers: list[CoverageBlocker] | None = None) -> str:
    """Extract magic values and constants from source code and blockers.

    Produces an AFL++/libFuzzer dictionary file (one token per line in
    the format: ``keyword="\\xDE\\xAD"``).

    Parameters
    ----------
    source_path:
        Path to the target source file to scan for literals.
    blockers:
        Optional list of CoverageBlocker objects from coverage analysis.

    Returns
    -------
    Dictionary file content as a string. Empty string if no tokens found.
    """
    tokens: dict[str, bytes] = {}  # name → raw bytes

    # 1. Extract from blockers' expected_value fields.
    if blockers:
        for i, b in enumerate(blockers):
            if not b.expected_value:
                continue
            val = b.expected_value.strip()
            raw = _literal_to_bytes(val)
            if raw and len(raw) <= 128:
                tokens[f"blocker_{i}_{b.blocker_type.value}"] = raw

    # 2. Scan source code for comparison literals.
    try:
    ...
    for m in _LITERAL_RE.finditer(source):
        ...
        if not lit:
            continue
        raw = _literal_to_bytes(lit)
        if raw and 1 <= len(raw) <= 128:
            # Use a sanitized key name.
            key = re.sub(r"[^a-zA-Z0-9_]", "_", lit)[:32]
            tokens[f"src_{key}"] = raw

    if not tokens:
        return ""

    # Format as AFL++/libFuzzer dictionary.
    lines: list[str] = ["# Auto-generated by CrashWise coverage analyzer"]
    for name, raw in tokens.items():
        escaped = "".join(f"\\x{b:02x}" for b in raw)
        lines.append(f'{name}="{escaped}"')
    return "\n".join(lines) + "\n"


def _literal_to_bytes(val: str) -> bytes | None:
    """Convert a source-code literal string to raw bytes."""
    val = val.strip()
    if val.startswith('"') and val.endswith('"'):
        return val[1:-1].encode("utf-8", errors="replace")
    if val.startswith("'") and val.endswith("'"):
        return val[1:-1].encode("utf-8", errors="replace")
    if val.startswith("0x") or val.startswith("0X"):
        try:
            ...
            return None
    if val.startswith(">= "):
        return None  # Length hints aren't tokens.
    try:
        n = int(val)
        if n == 0:
            return None
...
__all__ = ["analyze_coverage", "generate_dictionary"]
